...
player3 = animatedline('Marker','o','MarkerSize',20,'Color','g','LineStyle','none');
player3_v = animatedline('Color','g');
player4 = animatedline('Marker','o','MarkerSize',20,'Color','c','LineStyle','none');
player4_v = animatedline('Color','c');
...
bullet_3 = animatedline('Marker','o','MarkerSize',5,'Color','k','LineStyle','none');
bullet_4 = animatedline('Marker','o','MarkerSize',5,'Color','k','LineStyle','none');

terrain = animatedline([0 500 500 0 0 200 200 300 300 0 0 500 500 0],[0 0 500 500 0 0 500 500 0 0 300 300 200 200],'Color','k');
axis([0 500 0 500]);
...
for turn= 1:1000
   ...
   % Is enemy seen?
    ...
    clearpoints(player3_v)
    clearpoints(player4_v)
    clearpoints(bullet_1)
    clearpoints(bullet_2)
    clearpoints(bullet_3)
    clearpoints(bullet_4)
    ...
    addpoints(player3,p3_pos(1),p3_pos(2));
    addpoints(p_eye,p3_pos(1)+10*cos(p3_ang),p3_pos(2)+10*sin(p3_ang));
    addpoints(player3_v,p3_pos(1)+400*cos(p3_ang+ang_v3),p3_pos(2)+400*sin(p3_ang+ang_v3));
    addpoints(player3_v,p3_pos(1),p3_pos(2));
    addpoints(player3_v,p3_pos(1)+400*cos(p3_ang-ang_v3),p3_pos(2)+400*sin(p3_ang-ang_v3));
    if(bullet_3_exist==1)
        addpoints(bullet_3,bullet3_pos(1),bullet3_pos(2));
    end
    
    addpoints(player4,p4_pos(1),p4_pos(2));
    addpoints(p_eye,p4_pos(1)+10*cos(p4_ang),p4_pos(2)+10*sin(p4_ang));
    addpoints(player4_v,p4_pos(1)+400*cos(p4_ang+ang_v4),p4_pos(2)+400*sin(p4_ang+ang_v4));
    addpoints(player4_v,p4_pos(1),p4_pos(2));
    addpoints(player4_v,p4_pos(1)+400*cos(p4_ang-ang_v4),p4_pos(2)+400*sin(p4_ang-ang_v4));
    if(bullet_4_exist==1)
        addpoints(bullet_4,bullet4_pos(1),bullet4_pos(2));
    end
    drawnow;
